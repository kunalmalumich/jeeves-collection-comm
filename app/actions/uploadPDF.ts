...
export async function uploadPDF(formData: FormData) {
  ...
  try {
    ...
    const chunks = splitTextIntoChunks(parsedContent, 1000, 10);
...
Transactions:
List each transaction EXACTLY ONCE in the following strict format:
Date|Description|Amount|Category|UserName
Important rules:
- Use the pipe symbol (|) as the delimiter
- Do not repeat any transaction, even when discussing unusual or high-value items
- Each transaction should appear on a new line
- Maintain chronological order
- Do not include any commentary or additional formatting
- If any field is missing, use 'N/A' as placeholder
Balance Details:
