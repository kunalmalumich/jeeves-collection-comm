...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("POST request received:", { conversationSid, messageAuthor: message.author, messageBody: message.body })

    if (!conversationSid || !message) {
      console.error("Missing required parameters:", { conversationSid, message })
...
async function handleCustomerResponse(conversationSid: string, message: { author: string; body: string }) {
  ...
  try {
    ...
    const whatsappResult = await sendWhatsAppMessage(cleanPhoneNumber, messageResult.message, conversationSid)
    if (!whatsappResult.success) {
      throw new Error(`Failed to send WhatsApp message: ${whatsappResult.error}`)
    }
